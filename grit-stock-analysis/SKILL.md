---
name: grit-stock-analysis
description: Execute GRIT five-factor stock research pipeline. Creates stock folder, guides user through manual raw data collection, then runs the full HARNESS framework to produce a complete investment research report. Trigger when user requests stock analysis, asks to "跑一下GRIT", or mentions a stock they want to research.
version: 1.6
---

# GRIT 五要素股票研究 Skill

## 触发条件

当用户说「分析一下XXX股票」「跑GRIT」「研究一下XXX」「帮我看看这只票」时，加载此 skill。

## 系统定位

你不是传统写作助手。你是一个 **GRIT投研Agent**，具备：

- **RAG级检索**：地毯式扫描raw文件夹
- **产业洞察力**：超越机械填表，产出深度分析
- **数据可追溯**：所有数据标注来源；无则标「未识别」，禁编造

## 核心文件

Skill 内部已含核心文件（无需额外加载）：
- `references/HARNESS-股票研究执行框架-v1.34.md` — 执行框架
- `references/GRIT模板-五要素股票AI研究提示词-v1.34.md` — 报告模板
- `references/wind-excel-structure.md` — Wind/Choice Excel行区块结构（防止漏读关键比率）

---

## 工作流程总览

```
阶段0: 项目初始化 → 阶段1: Task理解 → 阶段2: 全量检索 → 阶段3: 信息提取
→ 阶段4: 深度分析 → 阶段5: 报告生成 → 阶段6: 质量校验
```

---

## 阶段0：项目初始化

### 触发条件
用户说「我想分析XXX股票，代码YYY」或「跑一下GRIT：XXX（代码）」

### 步骤

#### 0.1 创建项目文件夹

```
~/.hermes/data/grit/analysis/{股票名称}（{股票代码}）/
├── raw/           ← 用户手动放置原始资料
├── extracted.md   ← 阶段3产出：结构化素材
├── report.md      ← 阶段5产出：最终报告
└── sources.md     ← 阶段6产出：数据来源清单
```

**代码模板**：
```bash
mkdir -p "~/.hermes/data/grit/analysis/{股票名称}（{股票代码}）/raw"
```

#### 0.2 输出资料收集指引

创建并输出给用户一份清晰的资料清单：

```
## 📂 项目已创建：{股票名称}（{股票代码}）

### 请将以下资料放入 `raw/` 文件夹：

**必须项**（缺了这些基本没法写）：
| 优先级 | 资料类型 | 建议来源 |
|--------|---------|---------|
| 🔴 必须 | 最近3年年报（PDF） | 巨潮资讯网 / 港交所披露易 / SEC EDGAR |
| 🔴 必须 | 财务数据 Excel（利润表/资产负债表/现金流） | Wind / Choice / 同花顺 导出 |
| 🟡 重要 | 2-3份深度研报（PDF） | 东方财富研报中心 / 慧博 |
| 🟡 重要 | 行业研究报告 | 券商策略报告 / 行业协会 |
| 🟢 补充 | 公司调研纪要/访谈记录 | 雪球 / 格隆汇 / 调研纪要平台 |
| 🟢 补充 | 竞争对手相关资料 | 同上 |
| 🔵 加分 | 公司公告（重大合同/定增/股权激励等） | 巨潮资讯网 |

### 操作步骤
1. 打开文件夹：`~/.hermes/data/grit/analysis/{股票名称}（{股票代码}）/raw/`
2. 把下载好的文件全部扔进去（可按类型建子目录组织，如 `券商研报/`、`年报/`、`高临纪要/`、`财报/`，任意结构均可）
3. 回来告诉我「资料已放好，开始分析」

> 💡 资料越全，报告质量越高。实在找不到的我会标注"未识别"，不影响出报告。
```

#### 0.3 等待用户确认

用户回复「资料已放好」「开始分析」等触发 → 进入阶段1。

---

## 阶段1：Task理解

### 目标
扫描 raw 文件夹，解析 GRIT 模板结构，建立信息需求清单。

### 步骤

#### 1.1 扫描 raw 文件夹

```bash
find "~/.hermes/data/grit/analysis/{股票名称}（{股票代码}）/raw/" -type f | sort
```

列出所有文件：文件名、类型（Excel/PDF/其他）、大小。

#### 1.2 建立覆盖-缺口矩阵

对照 GRIT 模板信息需求，评估 raw 文件夹覆盖情况：

```
## 📊 资料覆盖情况

| 信息需求 | 覆盖状态 | 对应文件 |
|---------|---------|---------|
| 财务数据（利润表/BS/CF） | ✅ / ⚠️ / ❌ | xxx.xlsx |
| 年报管理层讨论 | ✅ / ⚠️ / ❌ | 2024年报.pdf |
| 行业规模及增速 | ✅ / ⚠️ / ❌ | xxx研报.pdf |
| 竞争对手数据 | ✅ / ⚠️ / ❌ | （缺失） |
| 调研纪要 | ✅ / ⚠️ / ❌ | xxx纪要.pdf |
| ... | ... | ... |

**图例**：✅ 有对应文件 | ⚠️ 部分覆盖/质量存疑 | ❌ 完全缺失
```

#### 1.3 输出缺口提示 + 询问用户

```
## ⚠️ 以下资料缺失，影响报告深度：

| 缺失项 | 影响板块 | 建议寻找 |
|--------|---------|---------|
| 竞争对手财务数据 | 竞争要素分析 | 同行业可比公司年报 |
| 行业规模预测 | 需求要素分析 | 券商行业深度报告 |
| ... | ... | ... |

### 请选择：
1. 🔄 **我去补资料** — 找到后放回 raw 文件夹，告诉我继续
2. ⏩ **跳过缺失项** — 缺失部分标注"未识别"，直接开始分析
3. 📝 **指定补充来源** — 告诉我从哪些URL/渠道获取特定数据
```

#### 1.4 等待用户决策

- 「补好了」→ 重新执行 1.1-1.2
- 「跳过，直接开始」→ 进入阶段2
- 指定来源 → 尝试自动获取（如网页抓取），获取不到则进入阶段2

---

## 阶段2：全量检索

### ⚠️ 前置硬约束：无一遗漏原则

**在执行任何检索之前，必须完成以下完整性检查：**

1. **全量文件清单**：`find raw/ -type f | sort` 列出每一个文件
2. **逐文件处理承诺**：所有文件都会被读取/提取/搜索，无一例外
3. **禁止主观跳过的判断**：
   - ❌ 「这个纪要是关于扫地机器人的，安克只是间接相关」→ 必须读！安克的竞争数据和市场份额可能在里面
   - ❌ 「已经有几份中文报告了，英文PDF跳过」→ 必须提取！海外投行有精确EPS预测和目标价演进
   - ❌ 「年报PDF可以用Excel替代」→ 必须搜索！年报有管理层的风险披露/分红政策/诉讼/股权激励等Excel没有的数据
   - ❌ 「这个文件看起来像重复/无关」→ 必须确认！用户放进raw文件夹的每一个文件都有其用意
4. **阶段2出口审计**（进入阶段3之前强制执行）：
   ```
   find raw/ -type f | sort  ← 列出全部文件
   逐文件确认：☑ 已读取 或 ☑ pdftotext+grep 或 ☑ 全行Excel读取
   任何未勾选的文件 = 阶段2未完成，禁止进入阶段3
   ```

### 目标
对 raw 文件夹进行**无一遗漏**的地毯式扫描，按 GRIT 框架提取信息。

### 检索策略

**维度一：文档类型全覆盖**

| 文档类型 | 检索重点 |
|---------|---------|
| Excel | ⚠️ **全量读取：从第1行到 `ws.max_row`**。Wind/Choice导出的财务摘要Excel通常80-90行，关键数据分散在多个区块：利润表摘要（行5-20）→ 资产负债表摘要（行22-40）→ 现金流量表摘要（行42-55）→ **关键比率**（行57-70，含ROE/ROIC/毛利率/净利率/资产负债率/资产周转率/现净比）→ 每股指标（行72-85）。**切勿只读前30行就判定数据缺失！** 尤其「关键比率」区块包含ROIC、季度毛利率、季度净利率等GRIT模板必需指标 |
| PDF年报 | 财务报表、管理层讨论、业务回顾、风险提示、附注 |
| PDF研报 | 盈利预测、目标价、行业数据、数据截止日期 |
| 纪要/访谈 | 括号补充说明、不确定信息标记、后续跟进事项 |

**维度二：隐性信息识别（按业务归类）**

对每个业务分别检查：
- 管理层表述的变化（不同时间点）
- 披露颗粒度变化（详细→笼统 或 反之）
- 资本开支方向与收入增长的时滞
- 客户结构变化与毛利率联动

**维度三：缺口二次确认**

扫描完成后对照信息需求清单再次输出覆盖状态，发现新的缺口及时询问用户。

---

## 阶段3：信息提取 → extracted.md

### 目标
构建结构化素材层。**规则：有则提取，无则标注"未识别"，严禁编造。**

### 输出结构（写入 extracted.md）

```markdown
# {股票名称}（{股票代码}）— 结构化素材

> 生成时间：{时间戳}
> 数据来源：raw/ 文件夹下 {N} 个文件

## 一、公司业务素材（严格按业务线分离）

### 业务1：[名称]
- **收入**：2023-2025年数据（来源标注）
- **产品**：形态/场景/技术/壁垒
- **客户**：结构/集中度/账期
- **销售**：模式/区域

#### 产业逻辑素材
- 需求函数变量：[具体数据或"未识别"]
- 价格弹性证据：[具体数据或"未识别"]
- 周期性证据：[具体数据或"未识别"]
- 供给约束：[描述或"未识别"]
- 价值链数据：[占比/议价能力或"未识别"]

#### 隐性趋势素材
- 管理层表述原文：[原文+时间或"未识别"]
- 披露颗粒度变化：[证据或"未识别"]
- 关联趋势数据：[数据或"未识别"]

#### 矛盾素材
[矛盾双方原文+来源或"未识别"]

### 业务2：[名称]
[同上结构...]

## 二、财务素材
[利润表/资产负债表/现金流量表 2023-2025，标注Excel单元格]

### 季度财务素材
| 季度 | 营收(亿) | 净利(亿) | 毛利率(%) | 净利率(%) | 来源 |
|:---:|:---:|:---:|:---:|:---:|:---|
| 20XXQ1 | | | | | Excel Sheet"季度数据" |

## 三、行业素材
[行业规模/增速/多源对比]

## 四、竞争素材
[对手财务/市场份额/竞争策略归类]

## 五、估值素材
[实时股价/市值/PE（必须实时抓取）]

## 六、风险素材
[四类风险描述/相关性/对冲措施/预警指标]

## 七、跨引用索引
| 引用ID | 业务分析结论 | 第二部分引用位置 | 引用要点 |
|:---:|:---|:---|:---|
| R1 | 业务A需求核心变量 | 需求要素→驱动因素 | 必须引用 |
| R2 | 业务A周期性判断 | 需求要素→驱动因素 | 必须引用 |
| ... | ... | ... | ... |
```

### 提取规则
- ✅ 每条信息附带来源（精确到文件+位置）
- ✅ 矛盾信息并列展示，不做取舍
- ✅ 产业逻辑素材按业务分离，禁止混写
- ✅ 无素材填"未识别"，禁编造
- ❌ 禁止复制整段原文
- ❌ 禁止用外部数据覆盖本地已有数据

### 缺口再次提示

提取过程中如发现无法从 raw 中获得的必要数据，中断并询问用户：

```
## ⚠️ 提取过程中发现以下数据缺失：

| 缺失数据 | 所在板块 | 影响 |
|---------|---------|------|
| 2025年毛利率 | 财务素材 | 无法判断盈利趋势 |
| 竞争对手ROIC | 竞争素材 | 无法完成竞争对比表 |

继续 / 跳过（标注"未识别"）/ 我去补数据？
```

---

## 阶段4：深度分析

### 定位
深度分析**不是独立输出**，而是为 GRIT 模板各板块提供分析素材。

### 步骤

#### 4.1 逐业务分析

对每个主营业务（收入占比从大到小）：
1. 读取 extracted.md 中该业务全部素材
2. **产业逻辑分析** → 200字+，每个bullet有证据支撑
3. **隐性趋势与矛盾洞察** → 100字+，有原文引用原文
4. **商业模式小结** → 一句话总结商业本质
5. 更新跨引用索引

#### 4.2 季度财务分析

1. 填充季度数据表（最近8个季度）
2. 增长类型判定（环比/同比/混合/下滑）
3. 识别成长限制性因素

#### 4.3 产业全景图景

200-500字，综合各业务产业逻辑和增长趋势。

#### 4.4 五要素归纳

按 GRIT 模板第二部分格式，强制引用跨引用索引。

---

## 阶段5：报告生成 → report.md

### 唯一标准

**严格按 GRIT 模板格式输出。模板文件：`references/GRIT模板-五要素股票AI研究提示词-v1.34.md`**

### 强制规则

1. 格式唯一来源：GRIT模板
2. 素材唯一来源：extracted.md
3. 深度分析嵌入模板指定位置
4. 五要素分析强制跨引用（R1, R2...）
5. 无素材处填"未识别"，禁编造
6. 可读性：通俗语言、每段≤150字、关键结论加粗

---

## 阶段6：质量校验

### 自动校验清单

#### 数据质量
- [ ] 财务数据标注Excel位置？
- [ ] 市场数据为实时抓取？
- [ ] 预测数据标注假设？
- [ ] 无素材处标注"未识别"？

#### 深度分析质量
- [ ] 每业务产业逻辑≥200字且有证据？
- [ ] 每业务隐性趋势如实填写？
- [ ] 季度数据如实填写？
- [ ] 增长类型判定有数据支撑？
- [ ] 产业全景≥200字？
- [ ] 敏感性覆盖≥3个假设？

#### 跨引用一致性
- [ ] 五要素引用跨引用索引？
- [ ] 无与第一部分矛盾的结论？

#### 延伸关注质量
- [ ] 所有标的来自 raw 资料？
- [ ] 每个标的标注出处？
- [ ] 无编造股票代码？
- [ ] 无合适标的时标注"未识别"？

#### 完整性
- [ ] GRIT模板所有表格已填充？
- [ ] 五要素完整？
- [ ] 投资建议明确？
- [ ] 买入PE<50倍或已注释原因？
- [ ] **季度表含「净利环比」行，且覆盖最新报告期？** (v1.5)
- [ ] **增长类型全量罗列（不匹配的保留不打勾）？** (v1.5)
- [ ] **PE-TTM为自行计算且全文唯一值？** (v1.5)
- [ ] **EPS/市值已统一量纲（券商预测转市值）？** (v1.5)

### 校验输出

```
## ✅ 质量校验结果

| 检查项 | 结果 | 说明 |
|--------|------|------|
| 数据来源追溯 | ✅ / ⚠️ / ❌ | |
| 深度分析质量 | ✅ / ⚠️ / ❌ | |
| 跨引用一致性 | ✅ / ⚠️ / ❌ | |
| 延伸关注质量 | ✅ / ⚠️ / ❌ | |
| 完整性 | ✅ / ⚠️ / ❌ | |

⚠️ 项已自动修复 / 需人工关注：...
```

---

## 最终产出

```
~/.hermes/data/grit/analysis/{股票名称}（{股票代码}）/
├── raw/                 ← 原始资料
├── extracted.md         ← 结构化素材（含跨引用索引）
├── report.md            ← 完整报告（GRIT模板格式）
└── sources.md           ← 数据来源清单
```

---

## 交互模式速查

| 场景 | AI行为 |
|------|--------|
| 用户说「分析XXX」 | 阶段0：创建文件夹 + 输出资料清单 |
| 用户说「资料放好了」 | 阶段1：扫描 + 输出覆盖-缺口矩阵 |
| 用户说「跳过缺失」 | 进入阶段2，缺失标"未识别" |
| 用户说「补好了」 | 重新扫描，更新覆盖矩阵 |
| 阶段3提取中缺数据 | 中断并询问 |
| 用户说「继续」 | 继续当前阶段，缺失标"未识别" |

---

## 特殊场景处理

### 数据不足
优先使用已有数据，缺失标注"基于外部来源"

### 数据矛盾
并列展示 + 分析原因 + 给出判断及置信度

### 早期/转型期公司
弱化历史财务，强化市场空间和技术路线；估值用PS/DCF

### 跨session续作
通过 extracted.md 恢复上下文，检查 raw 文件夹完整性后继续

---

## 注意事项 (Pitfalls)

1. **不要直接复制raw文件内容到report** — raw → extracted → report 是单向流水线
2. **实时市场数据必须抓取** — 股价/PE/市值禁止使用raw中历史数据，用web_search或API。详见 `references/real-time-data-sources.md`
3. **延伸关注标的只能来自raw** — 禁止从脑海中编造股票代码
4. **openpyxl 依赖** — 读取Excel需先安装 `pip install openpyxl --break-system-packages`，阶段2扫描Excel前先检查是否已安装，未安装则自动安装（1秒内完成）
5. **raw/ 支持子文件夹组织** — 用户可按类型建子目录（如 券商研报/、年报/、高临纪要/），`find` 递归扫描支持。Phase 0.2 的指引文案无需阻止子目录
6. **财务数据Excel优先** — 年报PDF只是补充，Excel数据更准
7. **季度数据缺失是常态** — 没有就标"季度数据缺失，仅基于年度数据判断"，不要编造
8. **用户可能跨session操作** — 每次续作先读 extracted.md 和 raw/ 文件列表恢复上下文
9. **🔴 Excel必须全量读取** — Wind/Choice导出的财务摘要Excel通常80-90行，前30行（利润表+资产负债表）之后还有现金流量表摘要和**关键比率**区块（ROE/ROIC/毛利率/净利率/资产负债率/周转率/现净比/季度数据/PE/PB/PS）。永远用 `max_row` → `for row in ws.iter_rows(min_row=1, max_row=ws.max_row)` 读到末尾，切勿中途截断后判定"数据缺失"。这是本次安克创新分析中实际踩过的坑——前30行看似只有营收/净利/资产，但关键比率全在50-87行。详见 `references/wind-excel-structure.md`
10. **🔴 Excel PE ≠ 实时PE** — Wind Excel底部估值指标（P/E TTM/PB/PS）基于Wind数据导出时的股价快照，可能与当前实时股价偏离。报告中应优先使用实时API抓取的估值数据，Excel估值指标仅供竞争对比时参考（因竞争对手同一快照时点，相对可比性仍有效）。报告中需标注数据时点差异。
11. **🔴 所有PDF券商报告必须先pdftotext再grep** — 每次执行阶段2时，所有raw/券商研报/下的PDF（含中英文学）都必须：①`pdftotext -layout` 提取全文到临时文件；②`grep -i '市场规模\|CAGR\|亿美\|market size\|target price\|EPS\|revenue.*20'` 搜索关键行业/估值数据。海外投行报告（BofA/UBS等）通常含精确EPS预测和目标价演进，国内券商深度报告常含招股书引用的第三方行业数据（如弗若斯特沙利文表）。切勿因为「已经有几份中文报告了」就跳过英文PDF或「封面看起来像重复内容」的PDF。
12. **🔴 竞争对手Excel也要读到底** — 绿联/传音/华宝等可比公司的Wind Excel同样在底部「关键比率」行含PE-TTM/PB/PS估值指标。读完目标公司Excel后立即同步读完所有竞争对手Excel，将估值数据填入竞争对比表。竞争对手PE是GRIT「估值逻辑与预测→可比公司估值对比」段落的必需数据。
13. **阶段2完成后的自我审计** — 在进入阶段3之前，做一次全量检查：`find raw/ -type f | sort` 列出所有文件，对照每个文件名确认是否已被读取/提取/搜索。特别是：所有.xlsx → 全行读取 √；所有.pdf → pdftotext + grep关键数据 √；所有.txt/.md → read_file完整读取 √。任何未确认的文件即为遗漏。此步骤防止两次被用户指出"这个文件你还没看"。

### 🔴 v1.5 新增 (2026-05-06 安克创新实战经验)

14. **🔴 禁止删除模板表格的任何维度** — GRIT模板中的表格（尤其是「季度财务追踪」表）有固定结构：营收/营收同比/营收环比/净利/净利同比/**净利环比**/毛利率/净利率。**禁止自行删除任何行或列**。如果某项数据缺失，标注"待查"或用已有数据推算，不得删除整行。本次安克创新分析中无故删除了「净利环比」行——这数据完全可以从已有季度净利直接算出。

15. **🔴 PE-TTM 必须统一且自算** — 报告全文只能出现**一个**PE-TTM值，且必须由AI自行计算：`PE-TTM = 实时股价 × 总股本 ÷ (最近4个季度归母净利之和)`。①先用最新季报EPS反推总股本（总股本 = 归母净利 ÷ EPS）；②TTM净利 = 最近4个报告季的归母净利（不含非经常归属）；③二者相除得实时PE-TTM。禁止混用Wind Excel快照PE（已过期）和不同来源的PE值导致全文出现34.97/23.81/25.9三个矛盾数字。

16. **🔴 「未识别」是最后手段，禁止滥用** — 遇到缺失数据时优先级：①用已有数据推算（如：期间费用率 ≈ 毛利率 − EBIT Margin）；②从券商研报提取（券商的利润表预测通常含销售/管理/研发费率明细）；③标注「推算值,置信度中」写入报告；④以上都失败才标「未识别」。本次期间费用率明明有毛利率45.07%+EBIT Margin 8.40%+信达证券Q1费率明细，却被标为「未识别」——属于明显的推算不足。

17. **🔴 EPS与市值必须统一量纲** — 券商预测表常以EPS呈现，报告中必须统一转为市值（EPS × 总股本 = 净利 → 净利 × PE = 市值）。总股本获取途径：①最新季报/年报的EPS和归母净利反推（Q1 EPS 0.8796，净利4.72亿 → 5.36亿股）；②年报「股本」章节；③Wind/东方财富实时查询。统一量纲后全文才可对话比较，不应出现「这边EPS 4.92那边市值660亿」的混乱。

18. **🔴 增长类型判定必须全量罗列** — 营收增长类型共10种（环比增长型/同比增长型/稳定型/趋势增长型/快速增长型/混合型/减速增长/加速增长/下滑型/剧烈波动型），盈利匹配共5种。**必须全部列出，仅在匹配的打✅，不匹配的保留不打勾**。这方便阅读者全面审视，避免因AI自行筛选而遗漏重要信号。

19. **🔴 季度财务追踪必须含最近报告期** — 季度表必须覆盖所有可用的报告季度。若raw中存在最新季报（如2026Q1），必须纳入，不得停留在上一个完整年度。每个季度至少8列（2年×4季=8），有新数据则扩展至9列+。

17. **🔴 不要在可推算时滥用未识别** — 未识别是最后手段不是偷懒工具。期间费用率可通过毛利率减EBIT margin近似推算，再结合季报中研发财务销售费率的分项变动趋势做出合理推断。推断必须标注推算及逻辑。
