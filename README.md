...
```
grit-stock-analysis/
├── SKILL.md                          ← 核心流程（市场无关的通用框架）
└── references/
    ├── HARNESS-股票研究执行框架-v1.35.md   ← 执行框架
    ...
    ├── real-time-data-sources.md      ← A股实时行情数据源
    ├── non-a-share-data-sources.md    ← 非A股实时数据源
    └── (v1.34 旧版及模板保留)
```
...
**v2.0** — 市场检测分流架构。A股/美股共享同一框架，差异仅在于 `references/market-*.md` 参数文件。
